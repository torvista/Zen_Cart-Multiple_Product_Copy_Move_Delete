 ...
 * @return string
 */
function list_subcategories($parent_id) // used in case $action = find, with subcategories
{
    global $db;
...
if (($action === 'find') || ($action === 'confirm')) { //validate form values from Preview and Confirm
    ...
    $within = ($_POST['within'] === 'name' ? 'name' : 'all'); // search filter in name, model or manufacturers only (name) or also descriptions (all)
    if (zen_not_null($keywords) && !zen_parse_search_string($keywords, $search_keywords)) {//steve merged
        $error = true;
    ...
    if ($copy_to === '' && $_POST['copy_as'] !== 'deleted' && $_POST['copy_as'] !== 'delete_one' && $_POST['copy_as'] !== 'delete_specials') {
        $error = true;
        $messages[] = ERROR_NO_TARGET_CATEGORY;
        ...
        if ($_POST['copy_as'] !== 'deleted' && $_POST['copy_as'] !== 'delete_one' && $_POST['copy_as'] !== 'delete_specials') {
            $check = $db->Execute("SELECT categories_id FROM " . TABLE_CATEGORIES . " WHERE categories_id = " . (int)$copy_to) . " LIMIT 1";
            if ($check->RecordCount() !== 1) {
                $error = true;
                $messages[] = ERROR_NOT_FOUND;
            } else {
                $check = "SELECT categories_name FROM " . TABLE_CATEGORIES_DESCRIPTION . " WHERE categories_id = " . (int)$copy_to . " AND language_id = " . (int)$_SESSION['languages_id'] . " LIMIT 1";
...
// bof: delete from one category
                    if ($_POST['copy_as'] === 'delete_one' && $query->RecordCount() === 1) { //if product found
                        ...
                        if ($chk_product->RecordCount() === 1) {
                            ...
                            $messages[] = 'ERROR cannot delete from ' . $_POST['category_id'] . ' multi ' . $product_multi->RecordCount() . '<br>';//todo
                        } else {
                            ...
                            $messages[] = 'Deleted products_id: ' . $id . ' from categories_id: ' . $_POST['category_id'] . ' master_categories_id: ' . $product_multi->fields['master_categories_id'] . $new_master_categories_id . '<br>';//todo
